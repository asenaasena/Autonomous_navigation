...
class AckermannDriveKeyop:
    ...
    def __init__(self, args):
        if len(args) == 1:
            ...
            max_speed = 100
            max_steering_angle = 375

        if len(args) > 2:
            cmd_topic = '/' + args[2]
        else:
            cmd_topic = 'steering_commands'
        ...
        for key in key_bindings:
            key_bindings[key] = \
                    (key_bindings[key][0] * float(max_speed) / 15,
                     key_bindings[key][1] * float(max_steering_angle) / 15)

        self.speed = 0
        self.steering_angle = 0
        self.motors_pub = rospy.Publisher(
            cmd_topic, AckermannDrive, queue_size=2)
        rospy.Timer(rospy.Duration(1.0/5.0), self.pub_callback, oneshot=False)
        self.print_state()
        self.key_loop()

    def pub_callback(self, event):
        steering_cmd_msg = AckermannDrive()
        steering_cmd_msg.speed = self.speed
        steering_cmd_msg.steering_angle = self.steering_angle
        self.motors_pub.publish(steering_cmd_msg)
    ...
    def finalize(self):
        ...
        self.settings = termios.tcgetattr(sys.stdin)
        steering_commands = AckermannDrive()
        steering_commands.speed = 0
        steering_commands.steering_angle = 0
        self.motors_pub.publish(steering_commands)
        sys.exit()
